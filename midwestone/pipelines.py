...
class MmidwestonePipeline:

    # Database setup
    conn = sqlite3.connect('midwestone.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `midwestone`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM midwestone WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO midwestone (date, title, link, content)"
